 ...
 * 칸의 3가지 상태
 * 	- 빈 칸: 참가자 이동 가능
 * 	- 벽: 참가자 이동 불가 / 1~9의 내구도 / 회전할 때 내구도가 1씩 깎임 / 0이 되면 빈 칸
 * 	- 출구: 참가자가 즉시 탈출
 * 참가자의 이동
 * 	- 1초마다 한 칸씩 동시에 이동
 * 	- 최단거리는 맨해튼 거리 
 * 	- 상하좌우 빈칸으로 이동 가능
 * 	- 움직인 칸은 머물렀던 칸보다 최단 거리가 가까워야함
 * 	- 움직일 수 있는 칸이 두개 이상이면 상하 우선
 * 	- 움직일 수 없으면 움직이지 않음
 * 	- 한 칸에 두명 이상 있을 수 있음
 * 미로의 회전
 * 	- 참가자의 이동 후 미로가 회전
 * 	- 한 명이상의 참가자와 출구를 포함한 가장 작은 정사각형을 잡음
 * 	- 2개 이상이면 좌상단 r좌표가 작은것이 우선, 그래도 작으면 c좌표가 작은 것이 우선
 * 	- 시계방향으로 90도 회전, 내구도가 1씩 깍임
 * 
 * 게임은 K초동안 반복. 모든 참가자가 시간안에 탈출하면 게임이 끝남
 * K초가 지나도 게임이 끝이 나지 않을 수 있음.
 * 
 * - 출력
 ...
 * - 필요한 메서드
 * 1. move() : 참가자들의 이동
 * 2. pickSquare() : 가장 작은 미로 선택
 * 3. rotaion() : 미로 회전 & 내구도 감소
 * 
...
//				System.out.println(i+1+"번 째 사람 탈출");
				peopleList.remove(i);
			}
		}
	...
	private static void rotaion() {
		...
		for (int[] people : peopleList) {
		    int r = people[0];
		    int c = people[1];
		    if(r>=squareR && r <squareR+squareL && c>= squareC && c<squareC+squareL) {
		    	// 1. 정사각형 내부 상대 좌표로 변환
		    	int nr = r - squareR;
		    	int nc = c - squareC;
		    	...
		    	// 2. 시계 방향으로 90도 회전
		    	int rotatedR = nc;
		    	int rotatedC = squareL - nr - 1;
		    	
		    	// 3. 절대 좌표로 변환
		    	people[0] = rotatedR + squareR;
		    	people[1] = rotatedC + squareC;
		    }
		}
	}
}
